...
export default function ChatInput({ onSend, onTyping, onStopTyping }) {
  ...
  return (
    <form onSubmit={submit} className="support-composer flex shrink-0 items-center gap-2 border-t border-slate-200 bg-white p-3">
      <label className="support-composer-tool grid h-11 w-11 shrink-0 cursor-pointer place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700" title={file?.name || t("chat.attachFile", { defaultValue: "Attach file" })}>
      ...
      <Button type="submit" className="h-11 w-12 shrink-0 rounded-xl p-0" aria-label={t("chat.sendMessage", { defaultValue: "Send message" })} disabled={!content.trim() && !file}>
