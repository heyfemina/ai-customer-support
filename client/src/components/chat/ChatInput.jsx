  ...
  return (
    <form onSubmit={submit} className="flex items-end gap-3 border-t border-slate-200 bg-white p-4">
      ...
      <textarea
        ...
        rows={1}
        className="min-h-10 flex-1 resize-none rounded-md border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-100"
        placeholder={t("chat.writeMessage")}
      />
      {file ? <span className="max-w-32 truncate text-xs font-semibold text-slate-500">{file.name}</span> : null}
      <Button className="h-10 w-10 p-0" aria-label="Send message">
