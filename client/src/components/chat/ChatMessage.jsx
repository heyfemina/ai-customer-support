...
export default function ChatMessage({ message, currentUserId }) {
  ...
  return (
    <div className={`flex ${mine ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[88%] overflow-hidden rounded-xl px-3.5 py-2.5 text-sm shadow-sm sm:max-w-[82%] xl:max-w-[76%] ${mine ? "rounded-br-md bg-blue-900 text-white shadow-blue-900/10" : "rounded-bl-md border border-slate-200 bg-white text-slate-700"}`}>
        {message.fileUrl ? (
          <div className="mb-2">
        ...
        <p className={`mb-1 text-[11px] font-bold uppercase ${mine ? "text-blue-100" : "text-blue-700"}`}>{senderName}{senderRole ? ` - ${senderRole}` : ""}</p>
        {hasTranslation ? (
