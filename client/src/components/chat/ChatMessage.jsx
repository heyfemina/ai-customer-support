...
export default function ChatMessage({ message, currentUserId }) {
  ...
  const mine = !message.isAI && (message.senderId === currentUserId || message.mine);
  ...
  const displayFileName = message.fileName || message.content || "Attachment";
  ...
  return (
    <div className={`flex ${mine ? "justify-end" : "justify-start"}`}>
      <div className={`max-w-[78%] overflow-hidden rounded-2xl px-4 py-3 text-sm shadow-sm ${mine ? "rounded-br-md bg-teal-600 text-white shadow-teal-900/10" : "rounded-bl-md border border-slate-200 bg-white text-slate-700"}`}>
        ...
        {message.isAI ? <p className={`mb-1 text-xs font-bold uppercase ${mine ? "text-teal-100" : "text-teal-700"}`}>AI Bot</p> : null}
        ...
        {hasTranslation ? <button className={`mt-2 text-xs font-semibold underline ${mine ? "text-teal-100" : "text-teal-700"}`} onClick={() => setShowOriginal(!showOriginal)}>{showOriginal ? "Show translation" : "Show original"}</button> : null}
