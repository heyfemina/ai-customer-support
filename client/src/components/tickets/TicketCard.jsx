  ...
  return (
    <Card className="p-5 transition hover:border-slate-300 hover:shadow-md">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-500">
        <Badge tone={ticket.priority === "URGENT" || ticket.priority === "HIGH" ? "red" : "slate"}>{t(`priority.${ticket.priority}`, { defaultValue: ticket.priority })}</Badge>
        <span className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1">{ticket.category}</span>
        <span className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1">{ticket.agent?.name || t("ticketsUi.unassigned")}</span>
        ...
        <span className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1">{t("common.updated")} {formatDate(ticket.updatedAt)}</span>
      </div>
      <Link to={`${basePath}/${ticket.id}`} className="mt-4 inline-block">
        <Button variant="secondary">{t("common.viewDetails")}</Button>
      </Link>
    </Card>
  );
}
