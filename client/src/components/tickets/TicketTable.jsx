...
export default function TicketTable({ tickets, basePath = "/admin/tickets", paginated = true }) {
  const columns = [
    ...
    { key: "priority", labelKey: "table.priority", align: "center", render: (row) => <Badge tone={row.priority === "URGENT" || row.priority === "HIGH" ? "red" : "slate"}>{row.priority}</Badge> },
    { key: "status", labelKey: "table.status", align: "center", render: (row) => <TicketStatusBadge status={row.status} /> },
    { key: "attachments", label: "Files", align: "center", render: (row) => <span className="inline-flex items-center justify-center gap-1 text-sm text-slate-600"><Paperclip className="h-4 w-4" />{row.attachments?.length || 0}</span> },
