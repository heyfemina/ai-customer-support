...
const demoAccounts = [
  ...
  { role: "Agent", email: "agent@example.com", password: "agent123" },
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-6 sm:px-7">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-wide text-blue-700">Secure access</p>
            <h1 className="mt-1 text-2xl font-bold text-slate-950">Sign in to your workspace</h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">Use one account. The app opens the correct admin, agent, or customer panel.</p>
          </div>
        </div>
      </div>
      <div className="space-y-5 bg-slate-50/60 p-6 sm:p-7">
        ...
        <p className="flex w-full items-start gap-2 rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm leading-6 text-blue-900">
          <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />
          <span>Role-based redirect is enabled after successful login.</span>
        </p>
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Demo login</p>
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            {demoAccounts.map((account) => (
              <button
                key={account.role}
                type="button"
                className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-900"
        ...
        <form className="space-y-5" onSubmit={submit}>
          ...
          <Button className="min-h-12 w-full rounded-lg" loading={loading}>Sign in and continue</Button>
        </form>
