    ...
    <Card className="p-6">
      ...
      <form className="mt-6 space-y-4" onSubmit={submit}>
        <label className="block">
          ...
          <input type="email" className="mt-1 h-11 w-full rounded-md border border-slate-200 px-3 outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-100" placeholder="you@example.com" value={email} onChange={(event) => setEmail(event.target.value)} required />
        </label>
        {message ? <p className="rounded-md bg-emerald-50 p-3 text-sm font-medium text-emerald-700">{message}</p> : null}
        {previewUrl ? (
          <a className="block rounded-md bg-sky-50 p-3 text-sm font-semibold text-sky-700" href={previewUrl} target="_blank" rel="noreferrer">
