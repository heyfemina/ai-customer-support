    ...
    <>
      ...
      <Card className="p-5">
        ...
        <form className="grid gap-5 lg:grid-cols-2" onSubmit={submit}>
          ...
          <label className="block lg:col-span-2"><span className="text-sm font-semibold">Upload file or image</span><input type="file" className="mt-1 w-full rounded-md border border-dashed border-slate-300 p-4" onChange={(event) => setFile(event.target.files?.[0] || null)} />{file ? <span className="mt-2 block text-sm font-semibold text-slate-500">Selected: {file.name}</span> : null}</label>
